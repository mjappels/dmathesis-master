...
of the solution.
\placehold{show how one recovers C-metric and Kerr from PD}
...
Teo (HT)~\cite{Hong:2003gx} realised that they could use the symmetry in
...
\todoruth{I am curious, how different are they?}same as those in the KW form of
the metric. To relate the two metrics, one can simply apply the set of
transformations from \cref{eq:cmet-transf}, and use the relations in
\cref{eq:cmet-transf-coefficients} between the coefficients to find, writing KW
parameters with a tilde (\~{}):\todoopt{some of the variables here may be
  confusing, $p,q,n$ are transf parameters, not sure if necess.}
\begin{align}
...
So far we have seen how the C-metric and its interpretation have evolved. We
will now dive in a little deeper and present some subtle aspects of the
solution. We will begin with the non-rotating form for simplicity, before
covering the generalised C-metric.
...
boundary~\cite{Kinnersley:1970zw, Plebanski:1976gy, Dias:2002mi,
  Griffiths:2005qp}. Although the C-metric is well-known among relativists,
...
Ignoring rotation for now, the general charged accelerating AdS black hole is
represented by the metric and gauge potential~\cite{Hong:2003gx}:
\begin{align}
...
\label{eq:cmetric1}
\end{align}
which is none other than \cref{eq:kinnersley-walker-metric} expressed using Hong
and Teo's parametrisation from \cref{eq:HTFandG} and written in spherical-like
coordinates\footnote{Explicitly: $t\to At$, $y\to1/Ar$, $x\to \cos\theta$ and
  $z\to \phi/K$}. Ordinarily, when discussing the C-metric, the azimuthal
angular coordinate $\phi$ is given an arbitrary periodicity, usually determined
such that one pole is regularised. In \cref{eq:cmetric1}, $\phi$ has a
$2\pi$-periodicity and any conical defect is tuned through the parameter
$K$. The conformal factor
...
review various constraints this spacetime imposes to the metric parameters. We
will then also briefly discuss the existence of conical defects in the
spacetime.
...
$r<r_\bd$ which are largely negative. Whether there is or is not an acceleration
horizon depends on the other parameters, and is viewed as a phenomenological
consequence of this choice. Finally, it is also be possible to extend this
spacetime beyond the horizons, which would allow one to consider further values
of $r$. Such a maximal extension of the C-metric in asymptotically AdS space was
provided by Krtou\v s~\cite{Krtous:2005ej}.
...
\end{equation}
Thus, this black hole will have an acceleration horizon if
$A > A_\mathrm{crit}$\footnote{Reverting back to a Kinnersley-Walker style
  parametrisation, where
  $f(r) = 1 - \frac{2m}{r} + \frac{e^2}{r^2} + r^2(\frac{1}{\ell^2}-A^2)$, this
  critical value is simply $A_\mathrm{crit} = 1/\ell$.} and no acceleration
horizon if $A<A_\mathrm{crit}$, the case $A=A_\mathrm{crit}$ is special and was
considered in \cite{Emparan:1999wa}.  Typically, in holographic applications,
the conical deficit of the C-metric is desired to be cloaked by a horizon
\cite{Hubeny:2009kz}, however, in what follows we will be interested in
thermodynamics, so to keep our thermodynamic variables unambiguous, we will take
$A<A_\mathrm{crit}$ so that we have a single horizon, that of the black hole,
and thus a well-defined temperature corresponding to the periodicity of
Euclidean time at the regular Euclidean black hole horizon.

While the above deals with possible sign flips from $f(r)$, there are also
negative regions for $g_{\theta\theta}$ and $g_{\phi\phi}$ that we must
consider. To preserve the signature, one must require that $g(\theta)>0$, which,
upon inspection of \cref{eq:fg11} or, equivalently, requiring that there
be no roots of
\begin{align}\label{eq:gx}
P(\xi) = 1+ 2mA\xi+e^2A^2\xi^2
\end{align}
over $-1<\xi<1$. To satisfy this condition we must require that 
\begin{align}
...
\todoopt{edit this para}
For small charge, this is equivalent to imposing an upper bound on the
acceleration parameter, while solutions with larger charge have no such
constraint; though these will be suppressed via cosmic censorship. It should
also be noted that larger values require nonzero charge for these conditions to
hold.
...
\begin{figure}
  ...
  \centering
    \includegraphics{eA-m1p2a0p6-small}
    \caption{\label{helloa}}
  \end{subfigure}\qquad
  \begin{subfigure}[b]{0.45\textwidth}
  \centering
    \includegraphics[width=\textwidth]{eA-m1p2a0p6-large}
    \caption{\label{hellob}}
  \end{subfigure}\\
  \begin{subfigure}[b]{\textwidth}
  \centering
    \includegraphics{eA-m1p2a0p6-large}
    \caption{\label{helloc}}
  \end{subfigure}
  \caption{\label{hello}}
\end{figure}
...
metric in this fashion, and having $g^{\mathrm{Sch}}_{\phi\phi}\sim K^{-1}$. For
$K>1$, the result is a black hole with a string running through its core. The
defect along both the $\theta = 0$ and $\theta= \pi$ axes is the same. The
C-metric has unequal deficits, and the resulting string tension imbalance is
what physically drives the acceleration.

The angular part of the metric in \cref{eq:cmetric1} near the poles is
\begin{align}
...
the Pleba\'nski-Demia\'nski metric,
\begin{align}
  ...
  B&=-\frac{e}{\Sigma r}\Big[dt-a\sin^2\theta \frac{d\phi}{K}\Big].
\label{eq:cmetric2}
\end{align}
...
positive over $0\leqslant\theta<\pi$. As these expressions are not particularly
transparent, we have plotted the corresponding regions in
\cref{fig:param-regions}. As previously stated, we see that for smaller
parameter values, there are upper bounds on the rotation and/or the
acceleration. Additionally, the inclusion of rotation, somewhat similarly to the
inclusion of charge allows one to access higher values of acceleration.
...
% \end{subfigure}
\caption{\label{fig:param-regions}Allowed values of $a$ and $A$ (in units of $\ell$ and $\ell^{-1}$ respectively) according to \cref{eq:conditions} (uncharged) are
...
\placehold{plot regions here}

We may also determine the conical deficits by looking at the region near each
pole. The string tensions for the rotating C-metric are given by
\begin{align}
...
\cite{Podolsky:2002nk}. For convenience, we shall rewrite the metric here and
...
string-like singularity along one polar axis attached to the black hole.  This
conical singularity can can be replaced by a finite width cosmic string
core~\cite{Gregory:1995hd} or a magnetic flux tube~\cite{Dowker:1993bt},
providing the force driving the acceleration.  The thermodynamics of these
objects has been the subject of recent studies with conflicting results.

On the one hand, a subset of us in \cite{Appels:2016uha,Appels:2017xoe}
presented a consistent set of thermodynamic variables for charged accelerating
black holes, but without rotation. The thermodynamic mass was found to be such
that the tension differential of the conical deficits on each axis satisfied
$\mu = MA$, dovetailing beautifully with the intuition of Newton's First Law and
the nett force on the black hole, however, this thermodynamic mass became
somewhat less intuitive in the presence of charge, and we were unable to use the
same method to get a satisfactory expression of $M$ for a rotating accelerating
black hole.  In addition, computing the action of the solution gives
\begin{align}
I = \frac{\beta}{2K} \left( m - 2mA^2\ell^2 
- \frac{r_+^3}{\ell^2(1-A^2r_+^2)^2} \right). 
\label{Caction}
\end{align}
that, while finite, gives a free energy inconsistent with the expressions found. 

On the other hand, in \cite{Astorino:2016ybm}, an alternate expression for mass
and temperature were found, however, the tension of the deficits was held fixed,
and it is unclear that the over-constrained system is fully
self-consistent.\todomike{A bit more here?}

We consider again here the thermodynamics of accelerating black holes in anti-de
Sitter spacetime, but with a fresh perspective based on a holographic
interpretation of the exact solutions. We address the above issues, finding a
distinct set of thermodynamic variables that are now consistent with the
gravitational action, and agree with both the conformal and holographic methods
of computing conserved charges. We therefore consider black holes with no
acceleration horizon, so that the temperature of the black hole is uniquely
defined, and the holographic computation and interpretation is clear.

An accelerating black hole in AdS has the geometry
\begin{align}
  \label{AdSC}
ds^2=\frac{1}{\Omega^2}\bigg[ -f(r)dt^2+\frac{dr^2}{f(r)}+r^2\Big(\frac{d\theta^2
}{g(\theta)}+g\theta\sin^2\theta\frac{d\phi^2}{K^2}\Big)\bigg],
\end{align}
where
\begin{align}
  f(r)&=(1-A^2r^2)\bigg(1-\frac{2m}{r}\bigg)+\frac{r^2}{\ell^2},\nn\\
g(\theta)&= 1+2mA\cos\theta, \qquad \Omega=1+Ar\cos\theta,
\end{align}
show clearly the black hole nature of the solution, with the $(1-2m/r)$ term in
the gravitational potential. $K$ encodes information about the conical deficits
on the north and south poles that have tensions given by~\cite{Appels:2017xoe}
\begin{align}
\label{mueq}
...
In \cite{Appels:2016uha,Appels:2017xoe} it was demonstrated that a ``First Law''
and Smarr relation held for this geometry in the form:
\begin{align}
\frac{m}{K} &= 2 T_\mathrm{H} S - 2 P V_\mathrm{H},\\
\delta (\frac{m}{K})  &= T_\mathrm{H} \delta S + V_\mathrm{H} \delta P 
- \lambda_+ \delta \mu_+ - \lambda_- \delta \mu_-,
\label{firstsmarr}
\end{align}
where $T_\mathrm{H}$ is the Hawking temperature computed from \eqref{AdSC} with
the time $t$, $S$ is the standard Bekenstein-Hawking entropy,
$P = 3/8\pi \ell^2$ is the thermodynamic pressure, and $V_\mathrm{H}$ the
thermodynamic volume of the black hole. $\lambda_\pm$ were the new variables
identified, conjugate to the tensions of the strings along each axis, and had a
natural interpretation in terms of the length from each pole to the origin plus
a correction.

\section{Computing the Mass}
\label{sec:mass-comput}

The original strategy in finding the thermodynamic variables appropriate to a
changing tension deficit in the presence of charge was to consider that the
thermodynamic mass might be a multiple of $m/K$ with $V$ and $\lambda_\pm$
adjusting as appropriate, however, the standard definitions of entropy and
temperature were assumed.  Interestingly, if we allow for a scaling of the
temperature simultaneously, we find that this first law, \eqref{firstsmarr}
holds for an arbitrary rescaling of mass, $M=\gamma(A\ell)m/K$, provided
$T=T_H (\gamma-A\gamma_{,A})$.  While interesting, there is no physical reason
to rescale the temperature, unless the time coordinate natural to the C-metric
is in fact not the correct time coordinate to consider. The appropriate time
coordinate is usually defined with respect to an observer at infinity, but since
we are in anti-de Sitter space our metric is divergent at the boundary, however,
we do have a natural procedure -- the Fefferman-Graham expansion -- with which
to explore the natural time coordinate. Before presenting this holographic
argument, we first give a .... \todomike{finish off this thought}

\subsection{The Ashtekar-Das Method}
\label{sec:ashtekar-das}

Setting $m=0$ removes the black hole, and leaves us with a Rindler solution. For
$A<A_\mathrm{crit}\leqslant \ell^{-1}$, there is no Rindler horizon, and
...
\end{align}
gives AdS in global coordinates, up to a rescaling of time:
\begin{align}
\label{gAdS}
ds_{AdS}^2= \Big(1+\frac{R^2}{\ell^2}\Big) \alpha^2
dt^2+\frac{dR^2}{1+\frac{R^2}{\ell^2}}  +R^2\Big(d\vartheta^2+\sin^2\vartheta
\frac{d\phi^2}{K^2}\Big)
\end{align}
with $\alpha = \sqrt{1-A^2 \ell^2}$. This suggests that the correct asymptotic
time therefore is not $t$ in the original C-metric \eqref{AdSC}, but rather
$\tau$, leading to a rescaling of the temperature:
\begin{align}
...
and as usual, entropy is one quarter of the horizon area:
\begin{align}
S=\frac{\mathcal{A}}{4}=\frac{\pi r_+^2}{K(1-A^2r_+^2)}\;.
\label{entropy}
\end{align}

This gives a very specific rescaling of the temperature, that would correspond
to a thermodynamic mass of $M = \alpha m/K$.  We will now provide three
arguments that this is indeed the correct value of the mass of the black hole.
...
regularisation of the AdS C-metric near the boundary through a conserved charge:
\begin{align}
Q(\xi )=\frac{\ell}{8\pi}\lim_{\bar{\Omega} \rightarrow 0}\int_\mathcal{M}
...
$\bar{g}_{\mu\nu} = {\bar{\Omega}}^{2}g_{\mu\nu}$, $\xi $ is a Killing vector,
and $N_{\mu }=\partial _{\mu }{\bar{\Omega}}$.  We pick
$\bar{\Omega}=\ell\Omega r^{-1} $, which provides a smooth conformal completion
in the limit $A=0$. The spacelike surface element tangent to $ \bar{\Omega}=0$
is \todomike{This was wrong - index down}
\begin{align}
  \label{surfel}
d\bar{S}_{\mu }=\delta^{\tau }_{\mu }\frac{\ell^{2}d\cos\theta d\phi}{\alpha K}
\end{align}
obtained by inserting $Ar\cos\theta=-1$ into the metric $\bar{g}$ and computing
the relevant determinant.  This yields
\begin{align}
  \label{confmass}
M = Q(\partial _{\tau})=\alpha \frac{m}{K}
\end{align}
for the mass, in agreement with the rescaled temperature, but in contrast to
previous results~\cite{Appels:2016uha,Astorino:2016ybm}\footnote{In
  ...
  indicated that $M=m/K$}. Note that $M$ vanishes for $A\ell\to 1$.
...
provided
\begin{align}
V &=\frac{4}{3}\frac{\pi}{K \alpha}\left[\frac{r_+^3}{1-A^2r_+^2}
+mA^2l^4(1-A^2r_+^2)\right],\nn\\
...
this system.  The action, including boundary counterterms
\cite{Balasubramanian:1999zv, Mann:1999pc}, is
\begin{align}
I[g]&=\frac{1}{16\pi}\int_{\mathcal{M}}d^{4}x\sqrt{-g}\left[ R+\frac{6}{\ell^{2}}\right]
+\frac{1}{8\pi}\int_{\partial \mathcal{M}}d^{3}x\sqrt{-h}\mathcal{K} \nn\\
&\hspace{10em}-\frac{1}{8\pi}\int_{\partial M}d^{3}x\sqrt{-h
}\left[ \frac{2}{\ell}+\frac{\ell}{2}\mathcal{R}\left( h\right) \right]
\label{action}
\end{align}
where $\mathcal{K}_{ab}$ is the extrinsic curvature of the boundary metric,
evaluated asymptotically in an appropriate coordinate system, defined
presently. $h_{ab}$ is the intrinsic metric on $\partial \mathcal{M}$, and
$\mathcal{R}$ its Ricci curvature. Computing the variation of this action gives
\begin{align}
...
where
\begin{align}
\kappa \mathcal{T}_{ab} = \ell \mathcal{G}_{ab}\left(h\right) 
-\frac{2}{\ell}h_{ab}-\mathcal{K}_{ab}+h_{ab}\mathcal{K},
\end{align}%

The holographic renormalization procedure requires new coordinates near the
boundary of AdS, typically parametrised by Fefferman-Graham coordinates, in
which
\begin{align}
...
We find that it is sufficient to expand the transformation asymptotically as
\begin{align}
\frac{1}{Ar}= -x-\sum_{n=1}^{4}F_{n}\left( x\right) \rho ^{-n} \quad
\cos \theta = x+\sum_{n=1}^{3}G_{n}\left( x\right) \rho ^{-n}
\end{align}
The $F_n$ and $G_n$ are fixed by the required fall-off properties of
\eqref{FGmetric}, apart from $F_1$ that feeds in to an overall arbitrary
conformal factor in the boundary metric:
\begin{align}
\gamma _{\tau \tau }& =-\omega (x)^{2} \ell^{-2}, \nn\\
\gamma _{\phi \phi }& =\frac{\omega (x)^{2}X(x)\alpha^{2}}{\left(
1-A^{2}\ell^{2}X(x)\right) K^{2}}~, \nn\\
\gamma _{xx}& =\frac{\omega (x)^{2}\alpha^{2}}{\left( 1-A^{2}\ell^{2}X(x)\right)
^{2}X(x)}~,
\end{align}
where $X(x) = (1-x^{2})\left( 1+2mAx\right)=\sin^2\theta g(\theta)$ and
\begin{align}
F_{1}(x) = -\frac{\left( 1-A^{2}\ell^{2}X(x)\right) ^{3/2}}{A\omega (x) \alpha}~.  \quad
\end{align}

\tcr{What are we trying to say here?}\tcb{Picking a $I$ between $x=-1$ and
  $x=1$, which yields
\begin{align}
\delta I=\frac{1}{\kappa }\int dtd\phi \frac{2m}{K^{2}\alpha}\left(
1-2A^{2} \ell^{2}\right) \delta K+O(\rho ^{-2}),
\end{align}
where to cancel the finite term one can set $\delta K=0$.}

The expectation value of the energy momentum of the CFT$_{3}$ can then be
calculated,
\begin{align}
...
U_{a}U_{b}+P\ell^{2}\gamma _{ab}+\pi _{ab},
\end{align}
with $U^{a}=\omega (x)^{-1}\delta _{\tau}^{a}$, $P=\rho _{E}/2$, and
\begin{align}
\pi _{a}^{b}=\frac{1}{\kappa }\frac{3mA^{2}X}{2\alpha^3\omega^{3}}\left(
1-A^{2}\ell^{2}X\right) ^{\frac32}
\left( \delta _{x}^{b}\delta _{a}^{x}-\delta _{\phi
}^{b}\delta _{a}^{\phi }\right)
\end{align}

The energy density is
\begin{align}
\rho _{E}=\frac{1}{\kappa }\frac{2m}{2K\alpha \sqrt{-\det \Sigma }}\left(
2-3A^{2}\ell^{2}X(x)\right),
\end{align}
and its integral yields the mass
\begin{align}
M=\frac{1}{\kappa }\int \rho _{E}\sqrt{-\det \Sigma }~dxd\phi
=\frac{\alpha m}{K}~,
\end{align}
in agreement with \eqref{confmass}.  Note that this calculation is independent
of the conformal frame. The non-trivial shear tensor implies that the dual fluid
is anisotropic, as expected from the strongly distorted boundary.

\section{Thermodynamic Phenomena}
\label{sec:phenomena}

\tcb{ An interesting thermodynamic quantity of physical interest is the
  isoperimetric ratio \todomike{Not sure about this bit}
\begin{align}
\mathcal{R}=\left( \frac{3V}{\omega _{2}}\right) ^{\frac13}\left(
\frac{\omega _{2}}{\mathcal{A}}\right) ^{\frac12}
\end{align}
where $V$ and $\mathcal{A}$ are the thermodynamic volume and horizon area of the
black hole. Most black holes satisify the \emph{reverse isoperimetric
  inequality} $\mathcal{R} \geqslant 1$~\cite{Cvetic:2010jb}, though notable
exceptions exist~\cite{Hennigar:2014cfa,Hennigar:2015cja,Brenna:2015pqa}.  For
the case at hand, $\omega _{2}=4\pi /K$, and we find
\begin{align}
\label{isoR}
\mathcal{R}^6=\frac{\left[1+\frac{A^2}{2}(1-A^2r_+^2) 
\big(1 + \frac{\ell^2}{r_+^2}(1 - A^2r_+^2)\big)\right]^2}{(1-A^2r_+^2)(1-A^2\ell^2)}, 
\end{align}
which, given that $Ar_+, A\ell \in [0,1]$, satisfies the inequality; it is
saturated for $A = 0$, the Schwarzschild-AdS solution.  }

Finally, let us return to the computation of the action \eqref{Caction}. Using
the correct time coordinate $\tau$ rescales the action by $1/\alpha$, and some
simple algebra then yields the expected result $F=I/\beta=M-TS$.  We plot this
quantity in figure \ref{fig:FE}.
\begin{figure}
  \centering
  \includegraphics[width=0.7\textwidth]{free-energy.pdf}
  \caption{\label{fig:FE}The red curve is the Schwarzschild-AdS case, illustrating
    the well-known Hawking-Page transition.  We see that this transition vanishes
    for sufficiently large $\mu$. \tcm{\bf[ I am not sure about those entirely
      negative curves...are they still in our approximation of small
      accelerations?]}}
\end{figure}
We see that for sufficiently small $\mu$ there is a Hawking-Page
transition. \tcm{\bf [More commentary here!  How can we understand this to be an
  HP transition, if indeed it is.  I could see a black hole plus string
  dissolving into radiation, but not the other way around.  Perhaps we should
  put in some speculations?]}


We have given a full and consistent description of the thermodynamics of an
accelerating black hole, reconciling discrepancies and conflicts that have
appeared in previous investigations of this system~\cite{Appels:2016uha,
  Appels:2017xoe,Astorino:2016ybm}.  We have also obtained the dual stress
energy tensor for the accelerating black hole, which corresponds to a
relativistic fluid with a non-trivial viscous shear tensor.  The shear is
proportional to the acceleration parameter and vanishes for $A=0$.

We note that proper identification of the normalization of the timelike Killing
vector was crucial in obtaining these results.  To see this, suppose we relate
the temporal coordinates of the metrics \eqref{AdSC} and \eqref{gAdS} via
$\tau=C t$ where $C=C(A,\ell)$ is some (unknown) function.  Proceeding as
before, we obtain \eqref{temp} with $\alpha\to C$ and \eqref{confmass} replaced
with
\begin{equation}
  \label{mass}
M=\frac{\alpha^2 m}{C K}
\end{equation}
and the remaining equations unchanged.  Fixing $\ell$ and $\mu_\pm$, we find
that the first law $\delta M=T\delta S$ holds only if $C=\alpha s(\ell)$, where
$s(\ell)$ is an arbitary function.  By varying $l$ together and enforcing the
Smarr relation we find that $s(\ell)$ must be constant; we can choose it to be
...
A further interesting subject is to study $A \ell > 1$ black holes, for which an
acceleration horizon exists.  We leave these problems for future work.
