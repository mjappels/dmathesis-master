...
negative curvature prevents the existence of an acceleration horizon. For
convenience, we shall rewrite the metric here:
\begin{align}
  ds^2=\frac{1}{\Omega^2}\left[ -
  f(r) dt^2
  +\frac{dr^2}{f(r)} + r^2 \Big( \frac{d\theta^2}{g(\theta)} 
...
In order to understand the structure of this accelerating black hole, it is
useful to consider the effects of the various parameters.  We suspect $m$ to be
...
was later discovered to be incorrect. \Cref{chap:holoTD} addresses the issue,
however these details were discovered in the later stages of the production of
this thesis. The results stated in this chapter hold despite this inconsistency.}
\begin{align}
  M = \frac{m}{K},
  \label{eq:massAcc}
\end{align}
...
\cref{chap:td-conical}, thus the only remaining parameter is $A$, which was
identified as the acceleration in \cref{sec:cmet-history} for the asymptotically
flat solution by studying the weak field limit $m\to 0$ and relating it to
the Rindler spacetime.

In order to understand the structure of the spacetime and the
relevance of $A$, first consider the Rindler space found by setting $m=e=0$ in
\cref{eq:cmetric3}:
\begin{equation}
