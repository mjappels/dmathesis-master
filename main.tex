...
$a\to0$, after which we are free to set $\epsilon=\gamma=1$ and $n=0$. The
...
\begin{gather}
  ...
  \mbox{with} \quad Q\to
  \frac{Q}{A^2r^4}, \quad \mbox{and} \quad \Xi=1-\frac{a^2}{\ell^2}+\mathcal{O}(A)
\end{gather}
...
time $\epsilon=1 + a^2/\ell^2$, which gives the Kerr-Newman-AdS metric.
...
polynomials which make up its components. Most computations with this metric and
its functions require knowledge of the latter's roots. An immediate example is
the range of the azimuthal coordinate --- covered in \cref{sec:conical} ---
which, if a regularity condition at one of the poles is imposed, depends on one
of the roots of $P$($\equiv G$).

Inspired by~\cite{Dowker:2001dg}, in 2003, Hong and Teo
(HT)~\cite{Hong:2003gx} realised that they could use the symmetry in
...
simple roots. Using the same notation, we keep $a_0 = -a_2 = 1$ and set
$a_1 = -a_3 = 2 m A$. The C-metric in Hong-Teo form is then still given by
