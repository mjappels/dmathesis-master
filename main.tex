...
\todoopt{need to contextualise, maybe tone down intro a little}
...
which do not satisfy the reverse isoperimetric inequality. In exploring possible
black hole solutions in four-dimensional Fayet-Iliopoulos gauged supergravities,
Gnecchi et al.~\cite{Gnecchi:2013mja} briefly presented a noncompact black hole
horizon with a finite area. It was later clarified in a letter by
Klemm~\cite{Klemm:2014rda} that this solution can be interpreted as the
\emph{ultra-spinning} limit of the Kerr-AdS solution, where the rotation
...
ultraspinning black hole was the first solution found to violate this
...
tension and decoupling $K$ as a physical parameter. This can be seen from the
expression for conical deficits \eqref{eq:deficit-al}: in this limit, $K$ is no
longer linked to the deficit, and therefore to the tension either. Now that we
also have the thermodynamic quantities, we see that some of these also diverge,
through their dependence on the function $\gamma\sim \Xi^{-1}$. In particular, the
angular velocity of the boundary diverges hence the `ultra-spinning' label
attributed to this limit. That $\gamma$ diverges poses the main problem for
resolving the first law.
...
\label{eq:dKdA}
\end{align}
It immediately follows, from substituting these into \cref{eq:firstlawinter},
that
\begin{multline}
...
this behaviour extends continuously to other critical limits, or it truly is
set-0 and remains disconnected from other geometries in physical parameter
