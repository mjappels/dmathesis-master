...
\begin{acknowledgements*}
  ...
  \" Ovg\" un, Pavel Krtou\v s and especially David Kubiz\v n\' ak, who has been
  extremely calm and patient and willing to explain many subtleties I had once
  failed to grasp. I would like to thank Aristos Donos for being my secondary
  supervisor and always being willing to have a quick chat. I would also like to
  thank Nigel Glover and Sebasti\' an Franco for being so encouraging during my
  years as an undergraduate and providing me with the necessary mentoring in the
  ...
  were not for the three of them. From Capriccio Mondays to sharing a house,
...
elsewhere. In \cref{cha:cmet}, we similarly review the C-metric, which describes
a uniformly accelerating black hole, its history, as well as certain derivations
leading to the metrics we use further. We also outline certain constraints that
must be taken into consideration when working with these solutions. In
\cref{cha:td-conical}, we develop the thermodynamical framework necessary to
admit varying conical deficits, and include the tensions of cosmic strings as
new extensive variables of our ensemble. This introduces a new potential, the
thermodynamic length. In \cref{cha:therm-accel-black}, we extend this
formulation to include accelerating black holes and consider independently
...
\emph{black hole entropy} proportional to the horizon area,
i.e.\ $S_{\mathrm{bh}}=\eta \mathcal{A}$, with $\eta$ being, at the time, an
...
Zel'dovich's work. Other notable contributions to the understanding of these
...
could~\cite{Hawking:1974rv,Hawking:1974sw}. Hawking himself later wrote, in
retrospect, about how he was initially embarrassed by the result and therefore
attempted to introduce various cutoffs in an effort to suppress these additional
modes~\cite{10.2307/24953849}. He eventually accepted the result, citing the
...
This derivation shows why the first law holds, at least for asymptotically flat
...
should be seen as more of an observation based on final expressions obtained
using independent definitions for mass, area, surface gravity and other
...
\end{align}
Introducing temporary coordinates $\rho^2 = 4(r-r_\mathrm{h})/f'(r_\mathrm{h})$ and
...
\begin{align}
  ...
  M &= F + TS + \sum_iP_i Q_i\nn\\
    &= 2TS + 2\Omega J + \Phi Q,
\end{align}
where the final line was obtained by explicitly computing the action for the
Kerr-Newman black hole. Historically, this was used to affirm the expression for
entropy using the Smarr formula from \cref{eq:smarrrelation}, however it could
equally be used to verify the converse.
...
cosmology with a small-but-nonzero cosmological constant (e.g.~ref.~\cite{Spergel:2006hy}). In the world of theoretical physics, arguably one
...
Computing the temperature from the metric~\eqref{eq:kerrAdS} in its Euclidean
...
sub-algebra deduced from the metric~\eqref{eq:kerrAdS}; Gibbons, Perry and
...
the two horizons coincide, or, if the black hole is charged, in the extremal
limit \cite{Mellor:1989gi,Mellor:1989wc}.
...
potential. From \cref{fig:grand-pot}, we see there is a critical value for this
...
\begin{figure}
  ...
  \caption{\label{fig:chargeplots}The free energy diagrams for the canonical
    ...
    $e=0.12\ell$ and $e = 0.2\ell$) are displayed over the uncharged case in
    black. The two intermdiary curves display the characteristic ``swallow
    tail'' behaviour. For the grand canonical ensemble, we have again reproduced
    three curves at fixed potential ($\Phi = 0.7$, $\Phi = 1.0$ and
    $\Phi = 1.2$) over the black uncharged plot. These figures have all been
    reproduced for $\ell = 1$.}
\end{figure}
...
gravitational physics is certainly exciting. The rich phase structure that black
holes possess, presented in the last section, is a powerful example of the
...
Eventually, in 1970, Kinnersley and Walker (KW) picked up the C-metric and
finally provided the interpretation of this solution as an \emph{accelerated}
black hole~\cite{Kinnersley:1970zw}. Let us now review how KW obtained their
version of the C-metric. The Levi-Civita solution is given in the form
\begin{align}
...
\begin{gather}
  ...
  z\to \eta z,
\end{gather}
with $\eta$, $\beta$ and $\varepsilon$ serving as real-valued parameters for the
transformation and as long as $G(x) \to \beta G(x)$, which imposes the following
relations between the coefficients of $G$ before (denoted by a tilde
$\tilde{}~$) and after the transformation:
\begin{subequations}
  \label{eq:cmet-coef}
  \begin{align}
    \beta a_0 &= \ta_0 + \ta_1 \varepsilon+ \ta_ 2 \varepsilon^2 + \ta_3 \varepsilon^3, \label{eq:cmet-coef1}\\
...
useful to first briefly review Rindler coordinates, which are simply a set of
...
\end{align}
we are able to recover the Rindler metric in 3+1-dimensional cylindrical
coordinates:
\begin{align}
  \label{eq:rindler}
...
tells us that these parameters are (locally) simple gauge choices, and we may
fix them without affecting the geometry. To recover the C-metric, we first take
the limit $a\to 0$, after which we are free to set $\epsilon=\gamma=1$ and
$n=0$. The C-metric, as written in \cref{eq:kinnersley-walker-metric}, is then
simply obtained by identifying $\tau=t$, $q=y$, $p=x$ and $\sigma=z$. The metric
\eqref{eq:PDrescaled} is actually known as the spinning C-metric (SC-metric), as
further work~\cite{Farhoosh:1980zz,Bicak:1999sa,Letelier:1998rx,Pravda:2002kj}
showed that while not analytically pleasant, this metric displays
characteristics of a rotating black hole while preserving its acceleratng
nature.
...
constraint~\eqref{eq:KWtoHT4} has only one solution for $\varepsilon$, which can
in turn be used to fully determine the remainder of the transformation. As it is
unpleasantly apparent, there is a sense in which the complicated root structure
of the KW parametrisation has been shifted and swept into the parameters
themselves.
...
better understood. We will now dive in a little deeper and present some subtle
...
show, we can recover the concept of equilibrium for slowly accelerating black
...
$\gamma$ would supposedly be sensitive to the direction of rotation as given by
...
\end{align}
Eliminating the integration function through a similar line of reasoning as
above leads to the correction terms already written above, with the additional
proviso that the correction to the gauge potential $\Phi_1 = 0$.
...
value of $M$ for which $\lambda$ becomes negative is larger.
% Comparing this
...
with its own temperature and it becomes clearer why such a formulation did not
exist.
...
potential issues that we brought up at the start of this chapter: whereas in the
...
weak field limit $m=0$ and exposing it as a reparametrisation of Rindler
...
\end{align}
resulting in the metric for anti-de Sitter space in global coordinates:
\begin{align}
   ds^2_\mathrm{AdS}= -(1-A^2\ell^2) \Big(1+\frac{R^2}{\ell^2}\Big) dt^2+\frac{dR^2}{1+\frac{R^2}{\ell^2}}  +R^2\Big(d\Theta^2+\sin^2\Theta
...
gives\footnote{More details regarding this computation are provided in
  ...
  an issue with the somewhat ambiguous scaling of the timelike killing vector
  which affects the computation by an overall multiplying
  factor. \Cref{cha:holoTD} addresses the issue, however these details were
...
independent of $\ell$. The method here is to seek a consistent set of
thermodynamic relations while maintaining the temperature and entropy as defined
earlier by the surface gravity and area respectively.
...
involved have proved too tall an order\footnote{A method for obtaining the
  ...
  results are summarised in the concluding chapter.}. We shall, however, revisit
the topic further in the chapter as we now take a turn to explore what these
findings may teach us about accelerating black holes.
...
We first explore the accelerating black hole in the canonical ensemble,
i.e.\ where the charge, $Q$, of the black hole is fixed, but we allow $M$ and
$\mu_-$ to vary.  In \cref{fig:TvMcharge}, we give a representative plot of
...
regime disappears, and the phase transition along with it. \Cref{fig:coexist}
...
Fayet-Iliopoulos gauged supergravities, Gnecchi \emph{et al.}\  briefly presented
a noncompact black hole horizon with a finite area~\cite{Gnecchi:2013mja}. It
was later clarified in a letter by Klemm that this solution can be interpreted
as the \emph{ultra-spinning} limit of the Kerr-AdS solution, where the rotation
parameter is taken to be critically large~\cite{Klemm:2014rda}. This limit only
...
thermodynamic implications of having such an extraordinary spacetime. In
particular, they sought to verify the reverse isoperimetric inequality
conjecture, which we first introduced back in \cref{sec:press-volume-black}, in
the context of these solutions. If the nondiverging area of a noncompact horizon
was the initial ``first of its kind'' for this solution, these papers
established the second such instance. The ultra-spinning black hole was the
...
will therefore be used to designate any black hole solution which exhibits
either a single or a pair of $2\pi$-deficit(s).
...
the Kerr-AdS metric, which was given in \cref{eq:kerrAdS}. In Boyer-Lindquist coordinates, it is,
\begin{align} \label{eq:kerrAdS-re}
...
is that the angular velocity evaluated on the boundary of the spacetime also
...
Let us return to the nonaccelerating solution \eqref{eq:kerrAdS-re} to review the
...
that the $2\pi$-deficit is clearly independent of $K$.
...
\begin{align}\label{eq:firstlawnr}
  \delta M = T\delta S + V\delta P - \lambda_+ \delta\mu_+ - \lambda_- \delta\mu_-,
\end{align}
...
&+\frac{K}{\Delta}\delta J\left((2-2e^2A^2-\Delta)\gamma_{a}-\frac{2ea}{\ell^2}(1-A^2\ell^2)\gamma_{e}\right) \nn\\
...
\begin{align}
\Omega_1 &= \frac{K}{\Delta}(2-2e^2A^2-\Delta)\gamma_a + \frac{2 a e
...
black holes are not isolated by virtue of the cosmic strings' presence, it is
possible to derive sensible-looking thermodynamics, although recent studies have
...
&- \frac{1}{8\pi}\int_{\partial \mathcal{M}}d^{3}x\sqrt{-h}
\left[ \frac{2}{\ell}+\frac{\ell}{2}\mathcal{R}\left( h\right) \right]\,,
\label{action}
\end{align}
where $\mathcal{K}_{ab} = \nabla_a n_b$, with $n$ the unit normal to the surface
$\partial \mathcal{M}$, is the extrinsic curvature of the boundary
metric, evaluated asymptotically in an appropriate coordinate system,
defined presently, and $\mathcal{K} = \mathcal{K}_{ab}h^{ab}$ is its trace on
the boundary. $h_{ab}$ is the intrinsic metric on
...
$X_3$ and $X_4$ in terms of $Y_n$ without having to solve differential
equations. With these functions we also find that $g_{\rho\rho}=\ell^2/\rho^2$
is automatically satisfied at leading order and we need only therefore eliminate
the two subleading terms. This is most easily achieved from here by fixing
$Y_2$, $Y_3$ and $Y_4$, which can be done once again first at next-to-leading
order for $Y_2$, at next order for $Y_3$ and again similarly at next order for
$Y_4$. With only seven of the eight functions determined, we find that
...
%\end{tabular}
\caption{\emph{Free energy.}  The red curve is the Schwarzschild-AdS case,
...
the action.  Alternate expressions for mass and temperature have been
...
\begin{align}
  V_1 &= \frac{4\pi
        m}{3K\alpha\Delta_+}\left(\Delta_+(A^2\ell^4\gamma-\alpha^2\ell^3\gamma_\ell)
        - 2\alpha^2 a^3 \gamma_a - a^2 - \Delta_+ A^2\ell^4\right),\nn\\
  \Omega_1 &= \frac{\alpha
             K}{\Delta_+}\left(-\frac{a}{\ell^2}+\Delta_-\gamma_a\right),\nn\\
  \lambda_\pm &= \frac{r_+}{\alpha(1\pm Ar_+)}- \frac{m\mp a^2 A}{\alpha \Delta_+}
                \mp \frac{A\ell^2\gamma}{\alpha} - \frac{2\alpha(m\mp a^2 A)
                a\gamma_a}{\Delta_+} \pm \alpha \gamma_A,
\end{align}
where
\begin{align}
  \Delta_\pm = 1 \pm \frac{a^2\alpha^2}{\ell^2}.
\end{align}
...
a^2A^2)$. Replacing $\gamma = (1+a^2A^2)/\Delta_-$, as provided by~\cite{Anabalon:2018aa},
...
\begin{align}
  ...
  - \frac{a^2}{r_+}+r_+^3\Big(\frac{1}{\ell^2}-A^2\Big)\right),\nn\\
  \Omega &= \frac{a K}{\alpha(r_+^2+a^2)} + \frac{a}{\ell^2}\frac{K}{\alpha\Delta_-},\nn\\
  V &= \frac{4\pi}{3K\alpha}\left(\frac{r_+ (r_+^2+a^2)}{(1-A^2r_+^2)^2} + mA^2
      \ell^4 + \frac{ma^2\alpha^2}{\Delta_-}\right),\nn\\
  \lambda_\pm &= \frac{r_+}{\alpha(1\pm Ar_+)}- m\frac{\Delta_+}{\alpha\Delta_-^2} \mp
                \frac{A\ell^2}{\alpha},
\end{align}
...
Equipped with these relations, we may now finally answer the question which led
to much of the work presented here. Is the critical rotating accelerating black
hole super-entropic? In other words, we must establish whether the critical
rotating C-metric violates the reverse isoperimetric inequality. As a reminder,
in the critical limit for this geometry (for $mA<1$ only),
\begin{align}
  \frac{a^2}{\ell^2}(1-A^2\ell^2)=1-2mA.
\end{align}
For small mass, the conditions that there be no acceleration horizon and that
there be an event horizon shielding the singularity prevent this limit from
being reached. For larger mass, the parametric restrictions that apply are
displayed in \cref{fig:aAm0717}, and the blue lines directly correspond to the
critical limit in each case. 
\begin{figure}
  \centering
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{aA-m0p7e0p0-small}
    \caption{\label{fig:aAm0717a}}
  \end{subfigure}
  \qquad
  \begin{subfigure}[b]{0.4\textwidth}
    \includegraphics[width=\textwidth]{aA-m1p7e0p0-small}
    \caption{\label{fig:aAm0717b}}
  \end{subfigure}
  \caption{\label{fig:aAm0717}Parametric restrictions that apply to the rotating
  accelerating black hole. The dashed lines correspond to acceleration horizons
  and the solid black lines to the extremal limit. The blue lines correspond to
  the critical limit in which one of the poles has a maximal conical
  deficit. In (a) $m=0.7\ell$, and in (b) $m=1.7\ell$.}
\end{figure}
The expression given by the reverse isoperimetric ratio is cumbersome, we
therefore plot the quantity $\mathcal{R}^6-1$, which is positive when the
inequality holds, in \cref{fig:finaliso}. As one may observe, the reverse
isoperimetric inequality is satisfied in both cases, provided the parametric
restrictions mentioned above are imposed. Our earlier conjecture, based on
perturbative arguments, that critical accelerated rotating black holes are not
super-entropic holds, and while we have not provided a mathematical proof that
this is always the case, \cref{fig:finaliso} strongly suggests that this
statement is valid in general.
 
\begin{figure}
  \centering
  \includegraphics{iso}
  \caption{\label{fig:finaliso}The reverse isoperimetric inequality states that
    the quantity $\mathcal{R}^6-1 > 0$ for all black holes. This is a plot of
    this quantity with respect to the rotation parameter $a$ at different values
    of the mass parameter $m$. The represented geometries have parametric
    restrictions which have been summarised in \cref{fig:aAm0717}.}
\end{figure}
% \appendix
%\include{appendix1}
%\include{appendix2}
