...
upon inspection of \cref{eq:cmetric1-fg} or, equivalently, requiring that there
be no roots of
...
\begin{equation}
  ...
  mA < \begin{cases} \frac{1+e^2A^2}{2} \quad &\mbox{if} \quad eA
    < 1\\
    eA \quad &\mbox{if} \quad eA > 1
    \end{cases}.
\end{equation}
...
\begin{align}
  m^2A^2 < A^2(a^2 + e^2) - \frac{a^2}{\ell^2} \quad \mbox{or} \quad A^2(a^2 +
  e^2) - \frac{a^2}{\ell^2} < mA < \frac12 \left(1 + A^2(a^2 +
  e^2)-\frac{a^2}{\ell^2}\right),
  \label{eq:conditions}
\end{align}
...
produces an overall force in the direction of the largest conical deficit, and
the geometry is described by the C-metric~\cite{Kinnersley:1970zw,
  Plebanski:1976gy, Dias:2002mi, Griffiths:2005qp, Podolsky:2002nk}, introduced
in \cref{sec:cmet}. Typically, C-metrics have both black hole and acceleration
horizons, however, for simplicity we wish to restrict ourselves to having only
the black hole horizon, in order that we have a well-defined temperature for the
space-time. Such a geometry is called the \emph{slowly accelerating C-metric}
...
the second such instance. The \emph{reverse isoperimetric
  conjecture}~\cite{Dolan:2013ft,Cvetic:2010jb} establishes an upper bound on
the entropy for any black hole given its thermodynamic volume, which is reached
only for spherical black hole solutions. The ultraspinning black hole was the
first solution found to violate this conjecture, leading the authors to impose
more stringent conditions under which the bound might be valid.
...
deficits in \cref{eq:deficits}, that this occurs when $g(\theta_-)=0$, or
...
presence of a (possibly dynamical) conical deficit\todoopt{caldarelli et
  al. etc.}. One key insight from this paper was that for this metric, the
boundary actually has a non-zero angular velocity,
\begin{align}
